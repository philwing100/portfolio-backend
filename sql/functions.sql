-- PostgreSQL functions (replaces MySQL stored procedures)
-- Run this after schema.sql
-- Call pattern: SELECT * FROM function_name($1, $2, ...)
--               SELECT function_name($1)  for void functions

-- ─── SESSION / AUTH ──────────────────────────────────────────────────────────

-- UpsertSessionWithRefresh
...
-- RotateRefreshToken
-- Finds the session row by old hash and atomically swaps to the new hash.
...
-- Fetch_list_object (current addressing scheme: parent_page + date)
...
-- Update_list_object — upsert by (user_id, parent_page, list_date)
...
-- Fetch_list (legacy title-based addressing)
...
-- Update_list — upsert by title, returns list_id
...
-- Delete_list — deletes items then the list (ownership-checked)
...
-- Update_list_item — insert item for a list
...
-- Delete_list_item — deletes all items for a list (ownership-checked via join)
...
-- Increment_streak — idempotent (once per day); returns affected_rows count
...
-- Update_streak — upsert by streak_id
...
-- Reset_streaks_by_IDs — resets current_streak to 0 for a comma-separated list of IDs
